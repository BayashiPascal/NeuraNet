...
"{\n \
  ...
  \"dataSetType\": \"0\",\n \
  \"desc\": \"Arrhythmia dataset, 279 inputs, 16 output\",\n \
  \"dim\": {\n \
    \"_dim\":\"1\",\n \
    \"_val\":[\"295\"]\n \
  },\n \
...
"    {\n \
      \"_dim\":\"295\",\n \
      \"_val\":[");
      for (int iIn = 0; iIn < NB_INPUT; ++iIn)
        fprintf(fp,"\"%f\",", dataset->_samples[iSample]._props[iIn]);
      for (int iOut = 0; iOut < NB_OUTPUT; ++iOut) {
        if (iOut == dataset->_samples[iSample]._cat)
          fprintf(fp,"\"1.0\"");
        else
          fprintf(fp,"\"-1.0\"");
        if (iOut < NB_OUTPUT - 1)
