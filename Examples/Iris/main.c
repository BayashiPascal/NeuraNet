...
int main(int argc, char** argv) {
  ...
  if (mode == 0) {
    ...
    FILE* fp = fopen("./iris.json", "w");
    fprintf(fp,
"{\n \
  \"dataSet\": \"Iris dataset\",\n \
  \"dataSetType\": \"0\",\n \
  \"desc\": \"Iris dataset, 4 inputs, 3 output\",\n \
  \"dim\": {\n \
    \"_dim\":\"1\",\n \
    \"_val\":[\"7\"]\n \
  },\n \
...
"    {\n \
      ...
      (dataset->_samples[iSample]._cat == setosa ? 1.0 : -1.0),
      (dataset->_samples[iSample]._cat == versicolor ? 1.0 : -1.0),
      (dataset->_samples[iSample]._cat == virginica ? 1.0 : -1.0));
